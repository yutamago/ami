...
function extractFunctionArgs(argsContent: string): string[] {
...
function extractFunctionSignatures(namespaceContent: string): string[] {
  ...
  for (let match of matches) {
    ...
    if (type === 'public:' || type === 'private:') continue;
...
function extractNamespaces(filePath: string): { name: string, functions: string[] }[] {
  const contents = fs.readFileSync(filePath, 'utf-8');
  let matches = contents.matchAll(namespaceHeaderRegex);
  const namespaces: { name: string, functions: string[] }[] = [];
  for (let match of matches) {
    const name = match.groups.name;
    ...
    while (scopeNum > 0) {
      ...
      if (contentChar === '}') scopeNum--;
      else if (contentChar === '{') scopeNum++;
    }
...
const namespaces = new Map<string, { name: string, functions: string[] }>();
