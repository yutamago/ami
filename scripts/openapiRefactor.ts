...
const prompt = (PromptSync as any).default({sigint: true, });
...
function apiPath2Ref(apiPath: string): string {
  let ref = '';

  const parts = apiPath.split('/').filter(x => x.length > 0);

  for (const part of parts) {
    if(ref.length > 0) ref += '_';

    if(part === '{id}') ref += 'index';
    else {
      ref += part;
    }
  }
...
function extractPaths(paths: object) {
...
function extractResourcesFromPath(content: object, refName: string, targetDir: string) {
